    ...
    <div class="card">
        <div class="card-body">
            @can('edit payment methods')
                <a href="{{ route('shop.admin.payment-method.edit', $paymentMethod) }}" class="btn btn-outline-primary">{{ __('Edit Payment Method') }}</a>
            @endcan

            @can('delete payment methods')
                {!! Form::open([
                        'route' => ['shop.admin.payment-method.destroy', $paymentMethod],
                        'method' => 'DELETE',
                        'class' => 'float-right',
