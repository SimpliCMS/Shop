    ...
    <div class="card-body">
        <h6 class="card-title">{{ __('Variants') }}</h6>
            @foreach($product->variants as $variant)
                <a href="{{ route('shop.admin.master_product_variant.edit', [$product, $variant]) }}" class="badge badge-secondary">
                    @if ($variant->hasImage())
            ...
            @endforeach
            @can('create products')
                <a class="btn btn-xs btn-success" href="{{ route('shop.admin.master_product_variant.create', $product) }}">
