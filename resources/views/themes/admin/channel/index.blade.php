    ...
    <div class="card card-accent-secondary">
        ...
        <div class="card-header">
            ...
            <div class="card-actionbar">
                @can('create channels')
                    <a href="{{ route('shop.admin.channel.create') }}" class="btn btn-sm btn-outline-success float-right">
                        {!! icon('+') !!}
                        {{ __('Create Channel') }}
                    </a>
                @endcan
                ...
                @foreach($channels as $channel)
                    <tr>
                        <td>
                            ...
                            @can('view channels')
                                <a href="{{ route('shop.admin.channel.show', $channel) }}">{{ $channel->name }}</a>
                            @else
                                {{ $channel->name }}
                            @endcan
                            </span>
                        ...
                        <td>
                            @can('edit channels')
                                <a href="{{ route('shop.admin.channel.edit', $channel) }}"
                                   class="btn btn-xs btn-outline-primary btn-show-on-tr-hover float-right">{{ __('Edit') }}</a>
                            @endcan

                            @can('delete channels')
                                {{ Form::open([
                                    'url' => route('shop.admin.channel.destroy', $channel),
