@extends('appshell::layouts.private')

@section('title')
    {{ __('Products') }}
@stop

@section('content')

    <div class="card card-accent-secondary">

        <div class="card-header">
            @yield('title')

            <div class="card-actionbar">
                @can('create products')
                    <div class="btn-group">
                        <a href="{{ route('shop.admin.product.create') }}" class="btn btn-sm btn-outline-success">
                            {!! icon('+') !!}
                            {{ __('New Product') }}
                        </a>
                        <button type="button" class="btn btn-sm btn-outline-success dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-expanded="false">
                            <span class="sr-only">Toggle Dropdown</span>
                        </button>
                        <div class="dropdown-menu">
                            <a href="{{ route('shop.admin.product.create') }}" class="dropdown-item">
                                {{ __('New Product') }}
                            </a>
                            <a href="{{ route('shop.admin.master_product.create') }}" class="dropdown-item">
                                {{ __('New Master Product') }}
                            </a>
                        </div>
                    </div>
                @endcan
            </div>

        </div>

        <div class="card-body">
            <table class="table table-striped table-hover">
                <thead>
                <tr>
                    <th>&nbsp;</th>
                    <th>{{ __('Name') }}</th>
                    <th>{{ __('Sales') }}</th>
                    <th>{{ __('Categorization') }}</th>
                    <th>{{ __('State') }}</th>
                    <th style="width: 10%">&nbsp;</th>
                </tr>
                </thead>

                <tbody>
                <?php $defaultThumbnail = 'data:image/png;base64,'; ?>
                @foreach($products as $product)
                    <?php $routeResource = is_master_product($product) ? 'master_product' : 'product'; ?>
                    <tr>
                        <td>
                            @can('view products')<a href="{{ route("shop.admin.$routeResource.show", $product) }}">@endcan
                                <img src="{{ $product->getThumbnailUrl() ?: $defaultThumbnail }}" class="mw-100" style="height: 2.5em;" />
                            @can('view products')</a>@endcan
                        </td>
                        <td>
                            <span class="font-lg mb-3 font-weight-bold">
                                @can('view products')<a href="{{ route("shop.admin.$routeResource.show", $product) }}">@endcan
                                    {{ $product->name }}
                                @can('view products')</a>@endcan
                            </span>
                            <div class="text-muted" title="{{ __('SKU') }}">{{ $product->sku }}</div>
                        </td>
                        <td>
                            <span class="mb-3">
                                {{ $product->units_sold }} {{ __('units sold') }}
                            </span>
                            <div class="text-muted" title="{{ show_datetime($product->last_sale_at) }}">
                                {{ __('Last sale') }}
                                {{ show_datetime($product->last_sale_at, __('never')) }}
                            </div>
                        </td>
                        <td>
                            <div class="mt-2">
                                @foreach($product->taxons as $taxon)
                                    <span class="badge badge-pill badge-secondary">
                                        {{ $taxon->name }}
                                    </span>
                                @endforeach
                            </div>
                        </td>
                        <td>
                            <div class="mt-2">
                                <span class="badge badge-pill badge-{{$product->is_active ? 'success' : 'secondary'}}">{{ $product->state->label() }}</span>
                            </div>
                        </td>
                        <td>
                            @can('edit products')
                                <a href="{{ route("shop.admin.$routeResource.edit", $product) }}"
                                   class="btn btn-xs btn-outline-primary btn-show-on-tr-hover float-right">{{ __('Edit') }}</a>
                            @endcan
                            @can('delete products')
                                {!! Form::open(['route' => ["shop.admin.$routeResource.destroy", $product],
                                        'method' => 'DELETE',
                                        'data-confirmation-text' => __('Are you sure to delete :name?', ['name' => $product->name])
                                    ])
                                !!}
                                <button class="btn btn-xs btn-outline-danger btn-show-on-tr-hover float-right">{{ __('Delete') }}</button>
                                {!! Form::close() !!}
                            @endcan
                        </td>
                    </tr>
                @endforeach
                </tbody>

            </table>

            @if($products->hasPages())
                <hr>
                <nav>
                    {{ $products->withQueryString()->links() }}
                </nav>
            @endif

        </div>
    </div>

@stop
