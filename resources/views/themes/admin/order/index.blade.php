            ...
            @foreach($orders as $order)
                <tr>
                    <td>
                            ...
                            @can('view orders')
                                    <a href="{{ route('shop.admin.order.show', $order) }}">{{ $order->number }}</a>
                                @else
                                    {{ $order->number }}
                                @endcan
                            </span>
                    ...
                    <td>
                        <div class="mt-2">
                                <span class="badge badge-pill badge-{{ $order->status->is_completed ? 'success' : ($order->status->is_cancelled ? 'warning': ($order->status->is_processing ? 'info' : 'secondary')) }}">
                                    {{ $order->status->label() }}
                                </span>
                        </div>
                    </td>
                    <td>
                        @can('delete orders')
                            {{ Form::open([
                                'url' => route('shop.admin.order.destroy', $order),
