        ...
        <div class="card-header">
            ...
            <div class="card-actionbar">
                @can('create shipping methods')
                    <a href="{{ route('shop.admin.shipping-method.create') }}"
                    ...
                    <tr>
                        <td>
                            ...
                            @can('view shipping methods')
                                    <a href="{{ route('shop.admin.shipping-method.show', $shippingMethod) }}">{{ $shippingMethod->name }}</a>
                                @else
                                    {{ $shippingMethod->name }}
                                @endcan
                            </span>
                        ...
                        <td>
                            @if($shippingMethod->zone_id)
                                @can('view zones')
                                    <a href="{{ route('shop.admin.zone.show', $shippingMethod->zone) }}" class="badge badge-pill badge-info">{{ $shippingMethod->zone->name }}</a>
                                @else
                        ...
                        <td>
                            ...
                            @else
                                @can('view carriers')
                                    <a href="{{ route('shop.admin.carrier.show', $shippingMethod->getCarrier()) }}">
                        ...
                        <td>
                            @can('edit shipping methods')
                                <a href="{{ route('shop.admin.shipping-method.edit', $shippingMethod) }}"
                                   class="btn btn-xs btn-outline-primary btn-show-on-tr-hover float-right">{{ __('Edit') }}</a>
                            @endcan

                            @can('delete shipping methods')
                                {{ Form::open([
                                    'url' => route('shop.admin.shipping-method.destroy', $shippingMethod),
