@foreach($taxons as $taxon)
    <div class="card-body">
        ...
        @can('edit taxons')
            <a href="{{ route('shop.admin.taxon.edit', [$taxonomy, $taxon]) }}">{{ $taxon->name }}</a>
        @else
            {{ $taxon->name }}
        @endcan
        ...
        <div class="d-inline card-actionbar-show-on-hover">
            @can('create taxons')
                <a href="{{ route('shop.admin.taxon.create', $taxonomy) }}?parent={{$taxon->id}}"
            ...
            @can('delete taxons')
                {{ Form::open([
                            'url' => route('shop.admin.taxon.destroy', [$taxonomy, $taxon]),
                            'class' => 'form',
    ...
    @if ($taxon->children->isNotEmpty())
        <div class="collapse multi-collapse" id="taxon-{{$taxon->id}}" data-toggle="collapse">
            <div class="card-body">
                <div class="card">
                    @include('shop-admin::taxon._tree', ['taxons' => $taxon->children])
                </div>
            </div>
        </div>
    @endif
